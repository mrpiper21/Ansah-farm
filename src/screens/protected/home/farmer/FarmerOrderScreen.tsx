...
const FarmerOrderScreen = () => {
  ...
  const { data: orders, isLoading, isError, error } = useQuery({
    ...
    queryFn: async () => {
      const response = await axios.get(`${baseUrl}/api/orders/farmer-orders/${user?.id}`)
  ...
  const renderOrderItem = ({ item }: { item: any }) => (
    <View
      ...
      // onPress={() => navigation.navigate('FarmerOrderDetails', { orderId: item._id })}
    >
      {/* Order Header */}
      ...
      <View style={styles.orderDetails}>
        <View style={styles.detailRow}>
          <IconSymbol name="calendar.circle.fill" size={16} color={Colors.light.text} style={styles.detailIcon} />
          <Text style={styles.detailText}>{formatDate(item.createdAt)}</Text>
        </View>
        
        <View style={styles.detailRow}>
          <IconSymbol name="cart" size={16} color={Colors.light.text} style={styles.detailIcon} />
        ...
        <View style={styles.detailRow}>
          {/* <IconSymbol name="mone" size={16} color={Colors.light.text} style={styles.detailIcon} /> */}
      ...
      <TouchableOpacity 
        ...
        onPress={() => navigation.navigate('farmerOrder-details', { id: item._id })}
      >
    ...
    <View style={styles.container}>
      ...
      {orders?.length === 0 ? (
        <View style={styles.emptyContainer}>
          <IconSymbol name="asterisk" size={48} color={Colors.light.text} style={{ opacity: 0.5 }} />
          <Text style={styles.emptyText}>No orders yet</Text>
          <Text style={styles.emptySubText}>When you receive orders, they'll appear here</Text>
        </View>
      ) : (
