...
const ChatScreen = ({ route }: Props) => {
  ...
  const receiver = foundChat?.participants.find((participant: any) => participant._id !== user?.id);
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      try {
        ...
        const sortedMessages = data.messages.map((msg: any) => ({
          ...msg,
  ...
  const renderMessage = ({ item }: { item: any }) => {
    ...
    const messageDate = item.createdAt instanceof Date ? item.createdAt : new Date(item.createdAt);
    
    return (
      <View style={[
        styles.messageContainer,
        item.sender === user?.id ? styles.userMessageContainer : styles.otherMessageContainer
      ]}>
        <View style={[
          ...
          <Text style={[
            styles.messageText,
            item.sender === user?.id ? styles.userMessageText : styles.otherMessageText
          ]}>
            {item.content}
          </Text>
          <View style={styles.messageMeta}>
            <Text style={[
              styles.timeText,
              item.sender === user?.id ? styles.userTimeText : styles.otherTimeText
            ]}>
            ...
            {item.sender === user?.id && (
              <Ionicons 
                ...
                size={14} 
                color={item.read ? Colors.light.primary : Colors.light.text} 
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerInfo}>
          <Text style={styles.receiverName}>{receiver?.userName || 'User'}</Text>
